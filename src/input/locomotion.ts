...
export function moveHorizontal(position: Vector3, forward: Vector3, axes: {x: number; y: number}, speed: number, dt: number, doorOpen: boolean): Vector3 {
  ...
  const valid = (px: number, pz: number) => px >= -1.28 && px <= 1.28 && pz >= .3 && pz <= (doorOpen ? 20.8 : 18.73) && !BLOCKERS.some(b => px > b.minX - .22 && px < b.maxX + .22 && pz > b.minZ - .22 && pz < b.maxZ + .22);
