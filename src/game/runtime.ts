...
export function createRuntime(canvas: HTMLCanvasElement, callbacks: {message: (text: string) => void; ended: (won: boolean, vr: boolean) => void; paused: () => void; status: (s: GameState) => void; fps: (value: string) => void}) {
  ...
  async function restart() {
    intro.stop();
    state = startGame(); nextStep = 0; previousTime = performance.now(); world.panel.setEnabled(false); audio.reset();
    if (xr?.active()) xr.reset(); else {camera.position.set(0, 1.65, 4); camera.rotation.set(0, 0, 0);}
    synchronize(); await audio.unlock();
  ...
  async function beginIntro() {
    audio.reset(); state = {...initialState(), phase: 'intro'}; world.panel.setEnabled(false);
    if (xr?.active()) xr.reset(); else {camera.position.set(0, 1.65, 4); camera.rotation.set(0, 0, 0);}
    await audio.unlock();
    await audio.preload().catch(() => callbacks.message('Nie wszystkie nagrania zostały wczytane.'));
    previousTime = performance.now(); intro.start(activeCamera());
    if (mode === 'desktop') await desktop.enable().catch(() => {});
  }
  async function finishIntro() {
    if (!intro.active) return;
    intro.stop(); audio.reset(); state = startGame(); previousTime = performance.now(); say('radio-start', true);
  }
  ...
  const visible = () => {
    if (document.hidden && mode !== 'menu') {state = {...state, paused: true}; audio.pause(); if (mode === 'desktop') {desktop.disable(); callbacks.paused();}}
    previousTime = performance.now();
  };
  document.addEventListener('visibilitychange', visible);
  const click = () => {if (mode === 'desktop' && !state.paused) void desktop.enable().catch(() => {});}; canvas.addEventListener('click', click);
  engine.runRenderLoop(() => {
    ...
    if (xr?.active() && ray) {const hit = scene.pickWithRay(ray, m => m !== dot && m.isEnabled()); dot.position.copyFrom(hit?.pickedPoint ?? ray.origin.add(ray.direction.scale(1.5)));}
    ...
    if (state.phase === 'threat' && !observed && !state.paused) {
      nextStep -= dt; if (nextStep <= 0) {audio.footstep(new Vector3(0, .1, state.enemyZ)); nextStep = .75;}
    }
    ...
    if (state.phase !== previous.phase && (state.phase === 'won' || state.phase === 'lost')) {
      endTime = now; audio.reset(); desktop.disable();
    ...
    synchronize(); audio.setListener(cam.position, cam.getForwardRay().direction, cam.getDirection(Vector3.Up()));
  ...
  return {
    ...
    async startVR() {
      ...
      xr ??= await createXR(scene, paused => {
        state = {...state, paused}; previousTime = performance.now();
        if (paused) audio.pause(); else void audio.unlock().catch(() => {});
      }, select, () => {
        const head = xr!.xr.baseExperience.camera;
        camera.position.copyFrom(head.position); camera.rotation.copyFrom(head.rotationQuaternion.toEulerAngles());
        mode = 'menu'; callbacks.paused();
      });
      // Request XR before asynchronous audio work, preserving the user activation.
      const resuming = ['intro', 'explore', 'knocking', 'threat'].includes(state.phase);
      await xr.enter(); mode = 'vr';
      if (resuming) {state = resumeOrStart(state); previousTime = performance.now(); await audio.unlock();}
      else await beginIntro();
    },
    async resumeDesktop() {state = {...state, paused: false}; mode = 'desktop'; previousTime = performance.now(); await audio.unlock(); await desktop.enable();},
    async menu() {intro.stop(); desktop.disable(); audio.reset(); audio.pause(); state = initialState(); mode = 'menu'; world.panel.setEnabled(false); if (xr?.active()) await xr.exit(); camera.position.set(0, 1.65, 4); camera.rotation.set(0, 0, 0);},
    restart, pause, skipIntro: finishIntro,
    getState: () => ({...state}),
    dispose() {engine.stopRenderLoop(); desktop.dispose(); xr?.dispose(); audio.dispose(); scene.dispose(); engine.dispose(); window.removeEventListener('resize', resize); window.removeEventListener('keydown', skipKey); document.removeEventListener('visibilitychange', visible); canvas.removeEventListener('click', click);},
  };
}
