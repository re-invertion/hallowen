...
import {initialState, startGame, enterKeyCell, takeKey, openDoor, takeFuse, openWardDoor, updateGame, dueKnocks, resumeOrStart} from './state';
...
describe('experience progression', () => {
  ...
  it('does not expose the key before the player enters the open cell', () => {
    const corridor = startGame();
    expect(takeKey(corridor).hasKey).toBe(false);
    const inside = enterKeyCell(corridor);
    expect(inside.keyCellVisited).toBe(true);
    expect(takeKey(inside).hasKey).toBe(true);
  });

  it('resumes a paused attempt without removing inventory or enemy progress', () => {
    const s = {...takeKey(enterKeyCell(startGame())), phase: 'threat' as const, paused: true, enemyZ: 6};
    expect(resumeOrStart(s)).toEqual({...s, paused: false});
    expect(resumeOrStart(initialState())).toEqual(startGame());
  });

  it('requires a key and restart state clears the whole attempt', () => {
    expect(openDoor(startGame()).doorOpen).toBe(false);
    expect(openDoor(takeKey(enterKeyCell(startGame()))).doorOpen).toBe(true);
    expect(initialState()).toMatchObject({phase: 'start', keyCellVisited: false, hasKey: false, doorOpen: false, hasFuse: false, wardDoorOpen: false, enemyZ: 2});
  });
  ...
  it('keeps the fuse and second latch progression after the new key-cell step', () => {
    ...
    const escapedDoor = openDoor(takeKey(enterKeyCell(s)));
    expect(escapedDoor.doorOpen).toBe(true);
